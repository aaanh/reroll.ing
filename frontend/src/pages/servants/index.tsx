...
const ServantIndexPage = () => {
  ...
  return (
    <>
      ...
      <main
        className={`relative min-h-screen w-full bg-gradient-to-b from-slate-950 to-pink-950/20 justify-center items-center text-slate-200 flex ${inter.className} flex-col p-8`}
      >
        <h1 className="text-4xl">Servant Index</h1>
        <Link
          className="text-white hover:text-blue-500 transition-all ease-in-out my-4"
          href="/"
        >
          <FaHome></FaHome>
        </Link>
        ...
        <div className="flex flex-wrap justify-center items-center">
          {servants.map((sv: Servant, idx: number) => (
            <SvSlot id={sv.collectionNo} key={idx} sv={sv}></SvSlot>
          ))}
        </div>
      </main>
    </>
...
export const SvSlot = ({ sv, id }: { sv: Servant; id: number }) => {
  ...
  return (
    <div
      id={id.toString()}
      className="flex flex-col justify-center items-center m-2 group"
    >
      <a
        href={`/servants/${sv.collectionNo}`}
        className={`hover:shadow-glow hover:border-blue-500 transition-all ease-in-out duration-300 relative text-center m-2 rounded-lg h-32 w-32 flex items-center justify-center border-2 p-1`.concat(
          " ",
          mapRarityToFrameColor(sv.rarity ?? 0),
        )}
      >
        {sv.face && (
          <Image
        ...
        <div className="absolute z-10 bg-black/50 px-2 capitalize text-sm bottom-1 left-50 rounded-full text-yellow-500">
          {mapRarityToText(sv.rarity)}
        </div>
        <div className="absolute top-1 left-1 bg-black p-1 rounded-xl font-bold text-sm">
          {sv.collectionNo}
        </div>
      </a>
      <div className="w-24 text-center text-sm capitalize font-bold">
        {sv.className}
      </div>
...
const GoToCollectionNo = ({
  ...
  return (
    ...
    >
      <input
        className="bg-blue-700 p-1 rounded-lg hover:bg-opacity-70 hover:cursor-pointer"
        type="submit"
        value="Jump To"
      ></input>
      &nbsp;
      <label>
        ...
        <input
          ...
          value={targetId}
          className="rounded-lg bg-transparent border text-white p-1"
          type="number"
          max={totalSv}
          min={1}
          id="targetId"
