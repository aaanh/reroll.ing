...
export default function Page() {
  ...
  async function getServantInfo() {
    try {
      const res = await fetch(`${API_SERVER}/servants/${String(collectionNo)}`);
      if (res.ok) {
        const _sv = (await res.json()) as unknown as { servant: Servant };
  ...
  return (
    <main
      className={`text-white ${inter.className} w-full min-h-screen flex flex-col items-center justify-center`}
    >
      <section className="sm:border border-white/50 rounded-lg sm:w-1/3 w-full flex flex-wrap sm:flex-row flex-col sm:relative sm:justify-normal sm:items-start justify-center items-center p-6">
        <div className="absolute right-1 top-1 rounded-full inline-flex justify-center items-center font-mono text-purple-300 font-bold">
          #{sv?.collectionNo}
        </div>
        ...
        <div className="text-center sm:text-left">
          <h1 className="text-green-500">{sv?.name}</h1>
          <h1>{`「${sv?.originalName}」`}</h1>
          <h2 className="capitalize text-white/50">{sv?.className}</h2>
      ...
      <div className="flex justify-between space-x-8 p-4">
        <Link
          href={`/servants/${(sv?.collectionNo ?? 1) - 1}`}
          className={`hover:text-blue-500 transition-all ease-in-out duration-300 ${sv?.collectionNo == 1 ? "pointer-events-none text-white/20" : ""}`}
        >
          <FaArrowLeft />
        </Link>
        ...
        <Link
          href={`/servants/${(sv?.collectionNo ?? 1) + 1}`}
          className={`hover:text-blue-500 transition-all ease-in-out duration-300 ${sv?.collectionNo == totalSv ? "pointer-events-none text-white/20" : ""}`}
        >
          <FaArrowRight />
        </Link>
      </div>
