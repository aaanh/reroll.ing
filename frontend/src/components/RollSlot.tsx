...
const RollSlot = ({ roll, handleShowSvInfoModal, currRollIdx }: { roll: Roll, handleShowSvInfoModal: (idx: number) => void, currRollIdx: number }) => {
  ...
  return <div className="flex flex-col justify-center items-center m-2 group">
    <button onClick={() => handleShowSvInfoModal(currRollIdx)} className={`hover:shadow-glow hover:border-blue-500 transition-all ease-in-out duration-300 relative text-center m-2 rounded-lg h-32 w-32 flex items-center justify-center border-2 p-1`.concat(" ", mapRarityToFrameColor(roll.servant?.rarity ?? 0))}>
